...
# Precise face oval contour (ordered clockwise around face edge)
FACE_OVAL_ORDERED = [
...
# Inner face landmarks to fill (forehead, eyes, nose, mouth, cheeks)
# These ensure the ENTIRE face interior is covered
FACE_INNER = [
    # forehead
...
class RambutPipeline(BasePipeline):
    def crop_part(self, image, landmarks):
        ...
        # ─── Crop: full width, from top to below chin ─────────
        crop_top = 0
        ...
        # ─── Build precise face mask ─────────────────────────
        # Use ordered oval contour as the outer boundary
        oval_pts = []
        ...
        # Fill the face contour precisely — no expansion, exact landmarks
        mask = np.zeros(crop.shape[:2], dtype=np.uint8)
        cv2.fillPoly(mask, [oval_pts], 255)

        # Hard edge blur — no feathering
